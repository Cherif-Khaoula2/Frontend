...
export class AjouterDossierComponent implements OnInit {
  ...
  onTypePassationChange(type: string) {
    ...
    this.fichierUploads = new Array(this.selectedFichiers.length).fill(null);
  ...
  tousLesFichiersRequisSontRemplis(): boolean {
  const fichiersArray = this.dossierForm.get('fichiers') as FormArray;
  if (!fichiersArray) return false;

  return fichiersArray.controls.every(control => !!control.value);
}
